...
const char months_of_year[] = {
    'J', 'a', 'n',  'u',  'a', 'r', 'y',  '\0', 'F', 'e',  'b',  'r',  'u',
    'a', 'r', 'y',  '\0', 'M', 'a', 'r',  'c',  'h', '\0', 'A',  'p',  'r',
    'i', 'l', '\0', 'M',  'a', 'y', '\0', 'J',  'u', 'n',  'e',  '\0', 'J',
    'u', 'l', 'y',  '\0', 'A', 'u', 'g',  'u',  's', 't',  '\0', 'S',  'e',
    'p', 't', 'e',  'm',  'b', 'e', 'r',  '\0', 'N', 'o',  'v',  'e',  'm',
    'b', 'e', 'r',  '\0', 'D', 'e', 'c',  'e',  'm', 'b',  'e',  'r',  '\0',
};
...
uint8_t ustrneq(const char *string1, const char *string2, uint8_t n) {
  ...
  uint8_t utz_i;
#endif
  for (utz_i = 0; utz_i < n && string1[utz_i] != '\0' && string2[utz_i] != '\0';
       utz_i++) {
...
char *ustrnreplace(char *dest, const char *src, char c, char *replacement,
                   uint8_t n) {
#ifndef UTZ_GLOBAL_COUNTERS
  uint8_t utz_i, utz_j;
#endif
...
uint8_t bin_to_bcd(uint8_t value) { return ((value / 10) << 4) | (value % 10); }
...
uint8_t dayofweek(uint8_t y, uint8_t m, uint8_t d) {
  ...
  y -= m < 3;
  d = ((UYEAR_TO_YEAR(y) + (UYEAR_TO_YEAR(y) / 4) - (UYEAR_TO_YEAR(y) / 100) +
        UYEAR_OFFSET / 400 + dayofweek_table[m - 1] + d) %
       7);
  if (d == 0) {
    return 7;
  } else {
...
#else
  if ((((UYEAR_TO_YEAR(y) % 4) == 0) && ((UYEAR_TO_YEAR(y) % 100) != 0)) ||
      ((UYEAR_TO_YEAR(y) % 400) == 0)) {
#endif
    return UTRUE;
  }
  return UFALSE;
...
uint8_t days_in_month(uint8_t y, uint8_t m) {
  static const uint8_t days_in_month_nonleap[13] = {0,  31, 28, 31, 30, 31, 30,
                                                    31, 31, 30, 31, 30, 31};
...
void unpack_rule(const urule_packed_t *rule_in, uint8_t cur_year,
                 urule_t *rule_out) {
  ...
  if (rule_in->on_dayofweek == 0) { // format is DOM e.g. 22
    ...
    first_dayofweek = next_dayofweek_offset(dayofweek_of_first_dayofmonth,
                                            rule_in->on_dayofweek);
    rule_out->date.dayofmonth = 1 + (7 * 3) + first_dayofweek;
    if (rule_out->date.dayofmonth + 7 <=
        days_in_month(cur_year, rule_in->in_month)) {
      rule_out->date.dayofmonth += 7;
    }
  } else { // format is DOW >= DOM e.g. Sun>=22
    dayofweek_of_dayofmonth =
        dayofweek(cur_year, rule_in->in_month, rule_in->on_dayofmonth);
    rule_out->date.dayofmonth =
        rule_in->on_dayofmonth +
        next_dayofweek_offset(dayofweek_of_dayofmonth, rule_in->on_dayofweek);
  }
...
void unpack_rules(const urule_packed_t *rules_in, uint8_t num_rules,
                  uint8_t cur_year, urule_t *rules_out) {
#ifndef UTZ_GLOBAL_COUNTERS
  uint8_t utz_i;
#endif
  uint8_t l = 0;
  ...
  for (utz_i = 0; utz_i < num_rules && current_rule_count < MAX_CURRENT_RULES;
       utz_i++) {
    ...
    if (cur_year >= rules_in[utz_i].from_year &&
        cur_year <= rules_in[utz_i].to_year) {
  ...
  // We override the "last" rule time of effect to be the start of the current
  // year
...
urule_t *get_active_rule(urule_t *rules, udatetime_t *datetime) {
  ...
  for (utz_i = 1; utz_i < MAX_CURRENT_RULES; utz_i++) {
    if (!RULE_IS_VALID(rules[utz_i]) ||
        udatetime_cmp(datetime, &(rules[utz_i].datetime)) < 0) {
...
char get_current_offset(uzone_t *zone, udatetime_t *datetime,
                        uoffset_t *offset) {
  urule_t *rule;
  if (last_zone != zone->src || last_year != datetime->date.year) {
    unpack_rules(zone->rules, zone->rules_len, datetime->date.year,
                 cached_rules);
    last_zone = zone->src;
...
void unpack_zone(const uzone_packed_t *zone_in, const char *name,
                 uzone_t *zone_out) {
  zone_out->src = zone_in;
  zone_out->name = name;

  zone_out->offset.minutes =
      (zone_in->offset_inc_minutes % (60 / OFFSET_INCREMENT)) *
      OFFSET_INCREMENT;
  zone_out->offset.hours =
      zone_in->offset_inc_minutes / (60 / OFFSET_INCREMENT);
